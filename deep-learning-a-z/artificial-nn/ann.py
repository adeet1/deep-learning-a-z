...
# Unlike the previous instance of KerasClassifier(), we don't specify a batch
# size and number of epochs. That's because these are the parameters we want to
# tune, so instead we will put those parameters into a GridSearchCV object that
# we will create.    
classifier = KerasClassifier(build_fn = build_classifier)

# This dictionary contains all combinations of the hyperparameters we want to
# try, and see which set of hyperparameters is the optimal one.
parameters = {"batch_size": [25, 32],
              "nb_epoch": [100, 500],
              "optimizer": ["adam", "rmsprop"]} # rmsprop is a very good optimizer for regular NNs
